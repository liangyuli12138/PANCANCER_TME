...
adata = "/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/06.cell2location/07.evaluate/01.cluster/cluster/D01972D1_cellbin.final.leiden.h5ad"
groupby = "celltype_merge"
adata = sc.read_h5ad(adata)

neighbors, shuffled_mean, zscore, nbor_pvals = compute_celltype_neighborhood(adata, groupby, niter=500, radius=40, permutation_method='local', oneside=True, n_jobs=20)
...
df.to_csv('out.nbor_pvals.csv')
df_to_merge.to_csv('out.zscore.csv')

df.index.name = 'celltype'
df_to_merge.index.name = 'celltype'

#df = df.reset_index(names ='celltype')
#df = df.reset_index()
#df = df.rename('celltype', axis='index')
#df_to_merge = df_to_merge.reset_index(names='celltype')
#df_to_merge = df_to_merge.reset_index()
#df_to_merge.rename('celltype', axis='index')
