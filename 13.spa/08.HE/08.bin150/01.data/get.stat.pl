...
<IN>;
while(<IN>){
if(/Unknown/ || /EC_Glomerular/ || /EC_Sinusoidal/ || /Hepatocyte/ || /Lymphoid_CD8_Tstr/ || /Lymphoid_MAIT/ || /Lymphoid_NKT/ || /Lymphoid_CD4_Tstr/ ){next};
@a=split(/,/);
$x=int($a[-5]/150)*150;$y=int($a[-4]/150)*150;
$z="$id"."_"."$x"."_"."$y";
$ha{$z}{$a[-3]}++;$haa{$z}++;
$hb{$z}{$a[-2]}++;$hbb{$z}++;
$hc{$a[-3]}=1;$hd{$a[-2]}=1;
