...
#inpute
RCTD_file <- paste0('/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/08.HE/03.TLS/umap/at/SS200000929BL_D2_TLS0.at')#cell2location结果文件
...
########################
output_path <- paste0(output_path,'/',outfix_sample,'.TLS0')
...
data <- subset(cellbin_data, subset = nFeature_Spatial>100 & percent.mt < 15 & nFeature_Spatial<3000 & area>50 & area<2500 & x>11941 & x<13030 & y>3260 & y<4348)
...
pdf(paste0(output_path,'/',outfix_sample,'.TLS0_firstType_sf.pdf'),w=16,h=8)
plot2
dev.off()
...
png(paste0(output_path,'/',outfix_sample,'.TLS0_firstType_sf.png'),res=300,w=8,h=8,units='in')
plot2
dev.off()
...
for(celltype in names(plot_list)){
  png(paste0(output_path,'/',outfix_sample,'_',celltype,'TLS0_Spatial.png'),res=300,w=8,h=8,units='in')
