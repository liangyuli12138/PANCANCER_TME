ls ../contour.diff/diff.Epithelium_Lymphoid3_*csv|perl -e 'print "gene\tcluster\n";while(<>){chomp;$id=$1 if(/(Epithelium_Lymphoid3\S+)\.csv/);open IN,$_;<IN>;$n=0;while(<IN>){@a=split(/,/);if($n<200){print "$a[1]\t$id\n";$n++}}}' > all.for.enrich.gene.list
