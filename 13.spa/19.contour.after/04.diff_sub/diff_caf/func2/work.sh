ls ../diff_caf/diff.Fibroblast_*csv |perl -e 'print "gene\tcluster\n";while(<>){chomp;$a=$_;if(!/Lymphoid3/){next};$a=~s/\.\.\/diff_caf\/diff\.//;$a=~s/\.csv//;open IN,$_;<IN>;$n=0;while(<IN>){@a=split(/,/);if($a[1]=~/MT\-/ || $a[1]=~/RPL/ || $a[1]=~/RPS/){next};$ha{$a}{$a[-1]}{$a[1]}=1}};for $i(keys %ha){$n=0;for $j(sort {$b <=> $a} keys %{$ha{$i}}){for $k(keys %{$ha{$i}{$j}}){if($n<200){print "$k\t$i\n";$n++}}}}' > all.for.enrich.gene.list
