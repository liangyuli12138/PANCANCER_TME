...
os.chdir("/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/06.cell2location/06.K_and_L/02.kidney_adjcent")

adata_concat = sc.read_h5ad("pancancer.ref.0723.raw.h5ad")
cellist = pd.read_csv("pancancer.ref.umap.0723.ka.obs.txt.list")
...
sc.tl.leiden(adata_concat, key_added='groups',resolution=0.6)

sc.tl.umap(adata_concat, min_dist=0.4)

adata_concat.write_h5ad("pancancer.ref.umap.0723.ka.h5ad")
adata_concat.obs.to_csv("pancancer.ref.umap.0723.ka.obs.txt")
adata_concat.var.to_csv("pancancer.ref.umap.0723.ka.var.txt")

sc.pl.umap(adata_concat, color='groups', save= ".ka.cluster.png")
sc.pl.umap(adata_concat, color='groups',  legend_loc='on data', save= ".ka..on.cluster.png")
...
sc.pl.umap(adata_concat, color='groups',  legend_loc='on data', save= ".ka.on.cluster.pdf")
