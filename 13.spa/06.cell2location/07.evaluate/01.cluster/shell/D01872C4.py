...
sc.pl.highly_variable_genes(adata_concat, save=".D01872C4.highly_variable_genes.pdf")
...
sc.pl.pca(adata_concat, save= '.D01872C4.adata.pca.png')
sc.pl.pca_variance_ratio(adata_concat, log=True, save= ".D01872C4.adata.pcavar_ratio.png")

sc.pp.neighbors(adata_concat, use_rep = "X_pca")
adata_concat.write_h5ad("D01872C4_cellbin.final.neighbors.h5ad")
adata_concat.obs.to_csv("D01872C4_cellbin.final.neighbors.obs.csv")

sc.tl.leiden(adata_concat, key_added='groups',resolution=1)

sc.tl.umap(adata_concat, min_dist=0.4)

adata_concat.write_h5ad("D01872C4_cellbin.final.leiden.h5ad")
adata_concat.obs.to_csv("D01872C4_cellbin.final.leiden.obs.csv")
adata_concat.var.to_csv("D01872C4_cellbin.final.leiden.var.csv")
sc.pl.umap(adata_concat, color='groups', save= ".D01872C4.cluster.png")
sc.pl.umap(adata_concat, color='groups',  legend_loc='on data', save= ".D01872C4.on.cluster.png")
