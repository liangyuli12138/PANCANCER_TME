...
os.chdir("/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/15.fig6/01.data")
...
adata = adata[cellist["cell"],:]
adata.obs = adata.obs.join(atlist)
adata.write_h5ad("pancancer.icar.all.cell.h5ad")
adata.obs.to_csv("pancancer.icar.all.cell.obs")

adata = sc.read_h5ad("pancancer.icar.all.cell.h5ad")
cellist = pd.read_csv("pancancer.icar.all.cell.obs.at.input")
atlist = pd.read_csv("pancancer.icar.all.cell.obs.at.at",index_col=0)
adata = adata[cellist["cell"],:]
adata.obs = adata.obs.join(atlist)
adata.write_h5ad("pancancer.icar.all.cell.h5ad")
adata.obs.to_csv("pancancer.icar.all.cell.obs")
