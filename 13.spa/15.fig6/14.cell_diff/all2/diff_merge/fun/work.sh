...
perl -e 'while(<>){chomp;@a=split(/,/);if($a[3]>1 && $a[5]<0.05 && $a[6]>0.1){print "$a[1],Epithelium_Malig_Lymphoid_1_2\n"}}' diff.Epithelium_Malig_Lymphoid_1_2.csv > all.for.enrich.gene.list
perl -e 'while(<>){chomp;@a=split(/,/);if($a[3]>1 && $a[5]<0.05 && $a[6]>0.1){print "$a[1],Epithelium_Malig_Lymphoid3\n"}}' diff.Epithelium_Malig_Lymphoid3.csv >> all.for.enrich.gene.list 
