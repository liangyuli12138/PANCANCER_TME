...
os.chdir("/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/12.TLS_celltype/01.cluster")
...
def process_sample(h5ad_file, csv_file, sample_id):
    ...
    # 筛选出符合条件的细胞
    filtered_cells = annotation[(annotation['celltype'] != 'other') & (annotation['celltype'] != 'malignant')]
   
    # 提取符合条件的cell的id
    cell_ids = filtered_cells['id'].tolist()
...
def process_samples(h5ad_directory, csv_directory):
    ...
    csv_files = [os.path.join(csv_directory, f) for f in os.listdir(csv_directory) if f.endswith('.all.at')]
    ...
    for h5ad_file in h5ad_files:
        ...
        if csv_file is not None:
            ...
            if merged_adata is None:
                ...
                merged_adata = merged_adata.concatenate(adata_subset)
...
csv_directory = "/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/11.icm/00.data/at.ori"
merged_adata = process_samples(h5ad_directory, csv_directory)

# 数据处理
sc.pp.normalize_total(merged_adata)
sc.pp.log1p(merged_adata)
sc.pp.highly_variable_genes(merged_adata, n_top_genes=3000)
