...
os.chdir("/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/12.TLS_celltype/04.cluster_pla/01.B")
...
def process_sample(h5ad_file, csv_file, sample_id):
    ...
    # 筛选出符合条件的细胞
    filtered_cells = annotation[((annotation['celltype'].str.contains('Lymphoid_B')) | (annotation['celltype'].str.contains('Lymphoid_Plamsa'))) & (annotation['LM'] == 'Lymphoid')]
        ...
        if csv_file is not None:
            ...
            if adata_subset is not None:
                if merged_adata is None:
                    ...
                    merged_adata = merged_adata.concatenate(adata_subset,batch_key=None)
...
csv_directory = "/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/12.TLS_celltype/04.cluster_pla/at"
...
run_leiden_clustering(merged_adata, leiden_resolutions)

genes=['MS4A1','CR1','CR2','FCER2','FDCSP','MKI67','CD22','BCL6','CD3D','FOXP3','GMDS','LMO2','LPP','SUGCT','AICDA','FRZB','BTNL9','CXCL13']
sc.pl.umap(merged_adata, color=genes, s=2, frameon=False, ncols=4, vmax='p99.9', na_color='grey' ,save=".st.tls.umap.markergene.png")
