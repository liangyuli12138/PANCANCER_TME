...
setwd("/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/12.TLS_celltype/05.ucell/02.ucell_scale/out/")
gene.set <- "/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/12.TLS_celltype/05.ucell/02.ucell_scale/tls.list"
...
SS200000929BL_D2 <- readRDS("/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/05.cell_correct/result/SS200000929BL_D2/SS200000929BL_D2_cellbin.final.rds")
data.name <- "Colorectal.SS200000929BL_D2_cellbin"
cell.subset <- SS200000929BL_D2
...
slide.seq <- AddModuleScore_UCell(slide.seq, features = markers,ncores=4,force.gc=T, slot= "data", normalize=TRUE, scale=TRUE)

marker.name <- colnames(slide.seq@meta.data)
index <- grep("_UCell",marker.name)
marker.name <- marker.name[index]

polygon_sf <- st_read("/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/09.cluster_immune/03.net/out/SS200000929BL_D2.original_polygons.shp")
...
for (ii in 1:length(marker.name))
{
	# Get the sequence depth plot for input data
        ...
        combined_plot <- plot2 + polygon_plot
        outpng <- paste(data.name,".",marker.name[ii],".SpatialFeaturePlot.png",sep="")
        png(outpng,width = 1200, height = 1600)
        print(combined_plot)
        while (!is.null(dev.list()))  dev.off()
...
csv.name <- "/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/12.TLS_celltype/05.ucell/02.ucell_scale/out/Colorectal.SS200000929BL_D2.UCell.score.csv"
