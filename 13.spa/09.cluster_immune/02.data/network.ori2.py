...
# 创建两个空列表用于存储Cluster_ori和Cluster_ex的ID
cluster_ori_new = []
cluster_ex_new = []

# 遍历每个细胞
for cell in range(len(adata.obs)):
    cell_id = adata.obs_names[cell]
    # 检查细胞是否在Cluster_ori中
    if cell_id in original_cell_ids:
        cluster_ori_new = original_cell_ids.index(cell_id)
    else:
        cluster_ori_new = "other"
    # 检查细胞是否在Cluster_ex中
    if cell_id in expanded_cell_ids:
        cluster_ex_new = expanded_cell_ids.index(cell_id)
    else:
        cluster_ex_new = "other"
    # 将Cluster_ori和Cluster_ex的ID添加到相应的列表中
    cluster_ori_new.append(cluster_ori_new)
    cluster_ex_new.append(cluster_ex_new)

# 将Cluster_ori和Cluster_ex的ID添加为adata.obs的新列
adata.obs['Cluster_ori'] = cluster_ori_new
adata.obs['Cluster_ex'] = cluster_ex_new

# 保存adata为h5ad文件
adata.write("output.h5ad")

# 保存adata.obs为obs.csv文件
adata.obs.to_csv("obs.csv", index=False)
