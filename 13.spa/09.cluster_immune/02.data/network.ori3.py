...
for cluster in clusters:
    ...
    gdf_original = gpd.GeoDataFrame(geometry=[original_polygon])
    gdf_original.to_file("original_polygon.shp", driver='ESRI Shapefile')
    # 将扩展多边形的坐标进行四舍五入
    expanded_polygon = transform(round_coordinates, unary_union(polygon.buffer(100)))
    # 将扩展多边形保存为shp文件
    gdf_expanded = gpd.GeoDataFrame(geometry=[expanded_polygon])
    gdf_expanded.to_file("expanded_polygon.shp", driver='ESRI Shapefile')
