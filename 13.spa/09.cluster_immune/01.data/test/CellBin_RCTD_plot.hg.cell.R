...
index <- "D01972D1"
outfix_sample <- "D01972D1"

#inpute
RCTD_file <- paste0('/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/09.cluster_immune/01.data/test/output.at.cell')#cell2location结果文件
...
#输出文件夹
output_path <- '/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/09.cluster_immune/01.data/test/plot/'


####58个亚类颜色设置#####
#color_use <- read.csv('/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/13.spa/08.HE/03.TLS/umap_sample/colour.list')
...
pdf(paste0(output_path,'/',outfix_sample,'cell_sf.pdf'),w=16,h=8)
plot2
dev.off()

plot2 <- ggplot() + geom_sf(data = sf,aes(fill=celltype),color=NA) +
             theme_void()+theme(plot.title=element_text(hjust=0.5,vjust=0.5))+ ggtitle(outfix_sample)+theme(legend.position="right")+
scale_fill_manual(values = cellType_cols)

png(paste0(output_path,'/',outfix_sample,'cell_sf.png'),res=300,w=8,h=8,units='in')
plot2
dev.off()
...
for(celltype in names(plot_list)){
  png(paste0(output_path,'/',outfix_sample,'_',celltype,'cell_Spatial.png'),res=300,w=8,h=8,units='in')
