...
class Pipeline(object):
    ...
    def set_config_file(self, cf):
        ...
        self.stitch_config['running_config']['num_threads'] = min(cpu_count // 2, self.stitch_config['running_config']['num_threads'])
        self.config_file['cell_correct']['num_threads'] = min(cpu_count // 2, self.config_file['cell_correct']['num_threads'])
    ...
    def _registration(self, ):
        ...
        try:
            ...
            if w_ > h_:
                thumb = f_resize(self.registrator.fov_transformed, (1500, int(h_ * 1500 / w_)))
            else:
                thumb = f_resize(self.registrator.fov_transformed, (int(w_ * 1500 / h_), 1500))
            ...
            with h5py.File(self._ipr_path, 'a') as conf:
                ...
                conf['Register'].create_dataset('MatrixTemplate', data=vision_cp[:, :2])
    ...
    def _cell_segmentation(self):
        ...
        try:
            ...
            with h5py.File(self._ipr_path, 'a') as conf:
                ...
                cell_seg.create_dataset('CellSegTrace', data=np.array(trace))
    ...
    def _cell_labeling(self):
        ...
        try:
            ...
            correct_mask, self.exp_matrix = cl.run_fast(distance=20)
        ...
        try:
            ...
            with h5py.File(self._ipr_path, 'r+') as conf:
                if 'Preview' in conf.keys():
                    del conf['Preview']
                conf.create_dataset('Preview', data=img)
    ...
    def regist_to_rpi(self):
        ...
        with h5py.File(self._ipr_path, 'r+') as f:
            ...
            f['TissueSeg'].create_dataset('TissueMask', data=self.encode_mask(tissue_mask))
            f['CellSeg'].create_dataset('CellMask', data=self.encode_mask(cell_mask))
        with h5py.File(self._ipr_path, 'r') as f:
