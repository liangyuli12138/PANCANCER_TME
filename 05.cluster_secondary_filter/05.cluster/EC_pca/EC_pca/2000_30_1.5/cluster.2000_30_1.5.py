...
os.chdir("/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/05.cluster_secondary_filter/05.cluster/EC_pca/EC_pca/2000_30_1.5")

adata_concat = sc.read_h5ad("pancancer.ref.0723.final.EC.h5ad")
cellist = pd.read_csv("/zfssz2/ST_TSCBI/P22Z10200N0433/USER/wubin2/wubin2/pancnew/05.cluster_secondary_filter/05.cluster/EC_pca/EC.input")
...
sc.pp.pca(adata_concat, n_comps=30, svd_solver='arpack', use_highly_variable=True)
...
sc.tl.leiden(adata_concat, key_added='groups_secori',resolution=1.5)

sc.tl.umap(adata_concat, min_dist=0.3)

adata_concat.write_h5ad("pancancer.ref.0807.final.EC_pca.umap.h5ad")
adata_concat.obs.to_csv("pancancer.ref.0807.final.EC_pca.umap.obs.txt")
adata_concat.var.to_csv("pancancer.ref.0807.final.EC_pca.umap.var.txt")
...
sc.pl.umap(adata_concat, color=[
                               'HEY1','IGFBP3','CD36','CA4','ACKR1','CCL21','PROX1','CXCL12','AMD1','CLU','ELN','LAMA5','EDN1','VCAM1','KDR',
                                'EDNRB','APLNR','INSR','ESM1','VWA1','COL4A1','COL4A2','POSTN','PGF','PXDN','MCAM','CLDN5','LAMB1','LXN'
                                                     ],
               s=2, frameon=False, ncols=4, vmax='p99.9', na_color='grey' ,save=".EC_pca.umap.markergene.png")
sc.pl.umap(adata_concat, color=[
                               'HEY1','IGFBP3','CD36','CA4','ACKR1','CCL21','PROX1','CXCL12','AMD1','CLU','ELN','LAMA5','EDN1','VCAM1','KDR',
                                'EDNRB','APLNR','INSR','ESM1','VWA1','COL4A1','COL4A2','POSTN''PGF','PXDN','MCAM','CLDN5','LAMB1','LXN'
                                                     ],
               s=2, frameon=False, ncols=4, vmax='p99.9', na_color='grey' ,save=".EC_pca.umap.markergene.pdf")
